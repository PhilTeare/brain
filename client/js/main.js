...
function Fact(inputArray, outputClass, weight){
	
	var factor = weight || 1;
	var out = {};
	out[outputClass] = factor;
...
function init(){
	net = new brain.NeuralNetwork([5,6]);
	var inputA = document.getElementById('inputA');
	var inputB = document.getElementById('inputB');
	
	inputA.addEventListener('change', handleFiles);
	inputB.addEventListener('change', handleFiles);
	...
	function handleFiles(e) {
	    ...
	    for (var i=0; i < e.target.files.length; i++){
		    ...
		    img.onload = function() {
		        ...
		        var scaledImage = downScaleImage(img, r);
		    	ctx.fillStyle = '#000';
		    	ctx.fillRect(0, 0, 250, 250);
		        ctx.drawImage(scaledImage, 0, 0);
		        imgData=ctx.getImageData(0,0,250,250);
		        out = {};
		        out[img.name]=1
		        fact = {input: imgData.data, output: out};
		        knowledge.push(fact);
		        
//		        var i,j,temparray,chunk = 4;
//		        for (i=0,j=imgData.length; i<j; i+=chunk) {
//		            temparray = imgData.slice(i,i+chunk);
//		            // do whatever
//		        }
//		        temparray;
...
scale = normaliseScale(scale);
 var sqScale = scale * scale; // square scale = area of a source pixel within
								// target
 ...
 var tx = 0, ty = 0, yIndex = 0, tIndex = 0; // target x,y, x,y index within
												// target array
 var tX = 0, tY = 0; // rounded tx, ty
 var w = 0, nw = 0, wx = 0, nwx = 0, wy = 0, nwy = 0; // weight / next weight
														// x / y
